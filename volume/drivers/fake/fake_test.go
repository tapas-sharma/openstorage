...
func testInitForCloudBackups(t *testing.T, d *driver) (string, string, *api.CloudBackupCreateRequest, *api.Volume) {
	...
	taskId, id, err := d.cloudBackupCreate(req)
	assert.NoError(t, err)
	assert.NotEmpty(t, id)
...
func TestFakeCloudBackupDelete(t *testing.T) {
	...
	taskId, backupId, createReq, _ := testInitForCloudBackups(t, d)
	_, err = d.kv.Get(backupsKeyPrefix + "/" + taskId)
	assert.NoError(t, err)
...
func TestFakeCloudBackupStatusWithMatchingVolume(t *testing.T) {
	...
	var credBackupReq *api.CloudBackupCreateRequest
	var vol *api.Volume
	var backupId, taskId string
	for i := 0; i < numbackups; i++ {
		// create backups
		taskId, backupId, credBackupReq, vol = testInitForCloudBackups(t, d)
	...
	assert.Equal(t, api.CloudBackupOp, resp.Statuses[taskId].OpType)
}
...
func TestFakeCloudBackupStateChange(t *testing.T) {
	...
	taskId, _, _, vol := testInitForCloudBackups(t, d)

	// Update element on db
	var elem *fakeBackups
	_, err = d.kv.GetVal(backupsKeyPrefix+"/"+taskId, &elem)
	assert.NoError(t, err)
	elem.Status.Status = api.CloudBackupStatusActive
	_, err = d.kv.Update(backupsKeyPrefix+"/"+taskId, elem, 0)
	assert.NoError(t, err)
	...
	assert.Equal(t, api.CloudBackupStatusActive, statuses.Statuses[taskId].Status)
	...
	assert.NoError(t, err)
	assert.Len(t, statuses.Statuses, 1)
	assert.Equal(t, api.CloudBackupStatusPaused, statuses.Statuses[taskId].Status)
	...
	assert.NoError(t, err)
	assert.Len(t, statuses.Statuses, 1)
	assert.Equal(t, api.CloudBackupStatusStopped, statuses.Statuses[taskId].Status)
